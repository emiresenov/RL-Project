...
class DQN(nn.Module):
    ...
    def act(self, observation, exploit=False):
        ...
        if exploit or U > self.eps:
          ...
          zeroOneActions = torch.argmax(action_values, dim=1)
          addTwoTensor = torch.Tensor([2]).to(device)
          actions = addTwoTensor + zeroOneActions
...
def optimize(dqn, target_dqn, memory, optimizer):
    ...
    subTwoTensor = torch.Tensor([2]).to(device)
    selectIndex = actions.unsqueeze(dim=1) - subTwoTensor
    selectIndex = selectIndex.to(torch.int64)
    q_values = torch.gather(action_values, 1, selectIndex)
